...
/* compute finish number as specified in SCFQ algorithm */
/*max(finish number of previous packet generated by the source, finish number of currently executing packet) + packet length */

if(Weight_flag == 0)
{
	if( Finish_num[source_id] > Curr_finish )
		temp_pkt->finish_num = Finish_num[source_id] + packetlen;
	else
		temp_pkt->finish_num = Curr_finish + packetlen; 
}
else
{
	if( Finish_num[source_id] > Curr_finish )
		...
		temp_pkt->finish_num = Curr_finish + packetlen/Weight[source_id]; 
}
